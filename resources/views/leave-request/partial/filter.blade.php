<div class="col-lg-8 col-md-6 col-sm-12">
    <div class="d-flex flex-column flex-lg-row justify-content-end gap-2">
        <div class="search-box col-lg-3 col-12">
            <form action="{{ route('leave-requests.index') }}">
                <div class="input-group">
                    <input type="text" class="form-control" name="search" value="{{ request('search') }}"
                        id="searchMemberList" placeholder="Cari data...">
                ...
                <li>
                    <label class="dropdown-item">
                        <input type="checkbox" name="status[]" value="pending"
                            {{ in_array('pending', request('status', [])) ? 'checked' : '' }} onchange="this.form.submit()"> Tertunda
                    </label>
                </li>
                <li>
                    <label class="dropdown-item">
                        <input type="checkbox" name="status[]" value="approved"
                            {{ in_array('approved', request('status', [])) ? 'checked' : '' }} onchange="this.form.submit()"> Disetujui
                    </label>
                </li>
                <li>
                    <label class="dropdown-item">
                        <input type="checkbox" name="status[]" value="rejected"
                            {{ in_array('rejected', request('status', [])) ? 'checked' : '' }} onchange="this.form.submit()"> Ditolak
                    </label>
                </li>
            </ul>
        </div>
