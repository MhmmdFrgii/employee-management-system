        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="container py-4">
                    <h1 class="h3 mb-4"><span><a class="text-black" href="{{ route('candidates.index') }}"><svg
                    ...
                    @if ($applicant)
                        ...
                        <!-- Include Modal -->
                        @include('candidates.partial.modal-aprrove')
                        @include('candidates.partial.modal-delete')

                        <div class="mt-4 text-center d-flex justify-content-center gap-3">
                            <form action="{{ route('candidates.update', $applicant->id) }}" method="POST">
                                @csrf
                                @method('patch')
                                <input type="hidden" name="applicant_id" value="{{ $applicant->id }}">
                                <input type="hidden" name="status" value="approved">
                                <a class="btn btn-primary btn-md d-flex align-items-center justify-content-center fw-semibold"
                                    href="#" data-id="{{ $applicant->id }}" data-bs-toggle="modal"
                                    data-bs-target="#ModalApproved">
                                    Lanjut
                                </a>
                            </form>
                            <form action="{{ route('candidates.update', $applicant->id) }}" method="POST">
                            ...
                            <button class="btn btn-danger btn-md" data-bs-toggle="modal"
                                data-bs-target="#ModalDelete{{ $applicant->id }}" type="button">Tolak</button>
                        </div>
                    @else
